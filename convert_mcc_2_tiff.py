...
def interpolate_nan(dose_container):
    ...
    In order to get rid of the NaNs that had no valid neighbor
    ...
    for i in range(dose_container.shape[0]):
        for j in range(dose_container.shape[1]):
            if invalid_mask[i, j]:
                ...
                if neighbors:
                    interpolated_dose_array[i, j] = np.mean(neighbors)
...
def write_tiff_file(dose_array, scale):
    """
    Parameters
    ----------
    dose_array : np.array  
   ...
   # Write TIFF file without ImageDescription
    with TiffWriter("n://Desktop//original_py.tif") as tif:
        tif.write(
    ...
    # This had to be implemented for VeriSoft and BeamAdjust 
    with Image.open("n://Desktop//original_py.tif") as img:
        ...
        img.save("n://Desktop//original_py.tif", tiffinfo=img.tag_v2)
...
if __name__ == "__main__":
    ...
    dose_image = bilinear_interpolate(dose_array, scale)
    dose_array_16bit = write_tiff_file(dose_image, scale)
